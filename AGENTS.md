...
> 플러그인/repo 이름 = **harnie**(개발 체인 스킬 허브). 개발 체인 6단계 = `requirements` → `software-design` → `software-design-review` → `implementation` → `implementation-review` → `acceptance-verification`. 요구사항·설계·검증은 스킵할 수 있다(예: 설계 → 구현 → 구현리뷰, 구현 → 구현리뷰). `dev`는 간단한 작업을 한 지시로 설계 → 설계리뷰 → 구현 → 구현리뷰 → 검증까지 돌리는 조합 스킬이고, 크거나 위험한 신호는 시작 전에, 커지거나 병렬 단위를 담은 설계는 설계 뒤에 넘긴다. 병렬은 설계 3절의 선택적 **병렬 단위**가 정하고, 하위 워크트리 디스패치·머지는 orca 소유다(`~/workspace/agent-ops/claude/orca-dispatch.md`). 체인과 무관한 스킬 = `pr-review` · `comment-resolve` · `deploy-approval` · `pr-delivery` · `quality-digest` · `confluence-doc`. 에이전트·커맨드·`instructions/`·run-state 훅은 없다 — 스킬 본문이 계약의 전부다. 훅은 `hooks/skill-guard.mjs` 하나로, Claude Code에서 체인이 대체하는 번들 스킬(`code-review`·별칭 `review`·`simplify`) 호출을 막는다.

**관측되지 않은 수치·조건은 설계·구현에 들어가지 않는다.** 기제가 물고 도는 런타임 값(타임아웃·재시도 횟수·주기·속도/동시성 상한, 분기를 거는 에러 코드와 상태)은 이 시스템의 측정에만 서고, 외부 레퍼런스도 요구사항이 값을 적었다는 사실도 그 자리를 대신하지 못한다. 요구사항이 기제 자체를 지목하지 않았는데 설계자가 골랐다면 기제 선택부터 외부 레퍼런스나 관측에 선다. 근거는 `docs/design-artifact-references.md` §21.
...
**리뷰어와 검증자는 별도 세션이고, 그 세션은 orca가 연다.** 코디네이터(산출물을 작성한 세션)가 orca로 다른 프로바이더의 대화형 세션 하나를 열고, 요청·결과는 파일로 주고받는다. 자율 라운드는 3회까지고 이후는 사용자 해제가 필요하다. 라운드 운영과 처분의 상세 규칙은 `software-design-review`·`implementation-review`·`acceptance-verification`이 각각 소유하며, 두 리뷰 스킬의 규칙은 같지 않다. 체인 산출물은 워크트리 루트 `_chain/` 하나에 둔다(한 워크트리에 한 체인, 커밋도 `.gitignore`도 하지 않는다). 디스패치·워크트리 수명주기는 orca 소유다. 강제 훅은 두지 않는다(근거: `docs/design-artifact-references.md` §14).
...
- **AGENTS.md가 정본**: 이 파일이 repo 지침의 정본이고 `CLAUDE.md`는 `@AGENTS.md` 한 줄로 그것을 import한다. repo 지침을 고칠 때는 이 파일만 고친다.
