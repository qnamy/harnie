...
**리뷰어와 검증자는 별도 세션이고, 그 세션은 orca가 연다.** 리뷰는 코디네이터(설계·구현을 한 세션)가 orca로 다른 프로바이더의 대화형 세션 하나를 열어 돌리며, 요청·결과는 파일로 주고받는다. 자율 라운드 상한은 3이고 4라운드부터는 요청 파일 첫 줄에 사용자 해제를 원문 인용한다. 한도에서 사용자에게 보고할 때는 왜 3라운드 안에 닫히지 않았는지를 — 결정이든, 안 고쳐진 결함이든, 검증 공백이든 — 먼저 답한다. 한 결정(검증 단계 포함)이 3라운드 연속 지적을 받으면 고치는 방안보다 제거하는 방안을 먼저 평가하고 그 비교를 사유에 적는다. 코디네이터 처분 표의 사유 칸은 **모든 처분에 필수**이고, `수용`에는 이번 라운드에 원 출처에서 확인한 것을 적는다. 코드와 계약이 환경 사실에서 어긋난 발견은 순서 있는 두 질문으로 가른다 — 막는 실패가 관측됐나(아니면 기제 제거), 계약 전제가 아직 참인가(참이면 코드 되돌리기, 거짓이면 계약 이동). 각 답은 그 자리에서 관측하고, 계약을 옮길 때만 원문 관측을 그대로 인용한다. 체인 산출물은 워크트리 루트 `_chain/` 하나에 두고(한 워크트리에 한 체인), 커밋도 `.gitignore`도 하지 않는다. 체인은 검증 파일 작성 또는 변경의 머지·릴리스로 끝나고, 그 뒤는 비워진 `_chain/`에서 시작하는 새 체인이다. 디스패치·워크트리 수명주기는 orca 소유다. 강제 훅은 두지 않는다(근거: `docs/design-artifact-references.md` §14).
