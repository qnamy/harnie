...
**리뷰어와 검증자는 별도 세션이고, 그 세션은 orca가 연다.** 리뷰는 코디네이터(설계·구현을 한 세션)가 orca로 다른 프로바이더의 대화형 세션 하나를 열어 돌리며, 요청·결과는 파일로 주고받는다. 자율 라운드 상한은 3, 이후는 사용자가 푼다. 체인 산출물은 워크트리 루트 `_chain/` 하나에 두고(한 워크트리에 한 체인), 커밋도 `.gitignore`도 하지 않는다. 디스패치·워크트리 수명주기는 orca 소유다. 강제 훅은 두지 않는다(근거: `docs/design-artifact-references.md` §14).
