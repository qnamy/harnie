...
- **Codex 무인 모드에서는 harnie run이 돌지 않는다(0.14.3 시점, 미해결).** codex의 `workspace-write` 샌드박스가 워크스페이스 안이라도 **`.git/` 하위 쓰기를 정책으로 막는다.** `captureTree`의 `git add -A`가 `objects/`에 임시 오브젝트를 쓰는 지점에서 `Operation not permitted`로 실패하고, `init`뿐 아니라 `runs`도 함께 실패한다(블로커 도출이 트리를 캡처하므로). 승인 경로가 있는 대화형 세션은 정상이므로 **Codex에서는 승인을 켜고 돌린다.** 프로브로 확인된 사실: `os.tmpdir()` 쓰기는 허용되고, 인덱스를 run root로 옮겨도 실패는 그대로다(원인이 인덱스가 아니라 오브젝트 DB). 검증된 우회는 오브젝트 저장소를 run root로 돌리고 원 저장소를 읽기 전용 alternates로 붙이는 것이다(`GIT_OBJECT_DIRECTORY` + `GIT_ALTERNATE_OBJECT_DIRECTORIES`) — 채택하면 캡처한 트리 SHA를 사람이 맨손 `git diff`로 못 읽게 되는 비용이 따른다. 무인 실행 수요가 실제로 생기면 그때 판단한다.
