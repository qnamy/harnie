...
- **Codex 무인 모드 — 고쳤으나 실런 미검증(0.14.6).** `.git/` 하위 쓰기를 막는 codex 샌드박스 때문에 캡처가 실패하던 것을 오브젝트 저장소 조건부 리다이렉트로 고쳤고(EPERM일 때만 발동, 대화형은 무동작), 리뷰어도 중첩 `codex exec`에서 네이티브 서브에이전트로 바꿨다(0.14.5). **끝단 증명이 없다** — 훅이 `TRUSTED_CLIS`를 자기 설치 루트로 계산해 미출시 런타임을 시험할 수 없어 카나리아를 릴리스 뒤로 미뤘다. 설치본으로 무인 S run 1건이 `complete:true`로 닫히는 것을 확인하면 이 항목을 지운다.
