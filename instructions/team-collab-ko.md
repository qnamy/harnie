...
Agent Teams 범용 규칙 — 라우팅(5문 기준), 하드 룰, 팀원 축, 템플릿, 알려진 제약 — 은 `~/workspace/agent-ops/claude/agent-teams.md`에 있으며 **여기서 반복하지 않는다**. 그 파일을 먼저 읽어라. 이 문서는 harnie 특화 사항만 얹는다. 충돌 시 팀 메커니즘은 범용 파일이, harnie 파이프라인 통합은 이 파일이 소유한다.
...
팀을 만들기 전 셋 다 성립해야 하며, 단계 경계에서 확인한다:
...
셋 다 성립하기 전까지 이 프로파일은 비활성이며, `harnie:dev`는 기존 단독 디자이너 경로로 실행한다.
