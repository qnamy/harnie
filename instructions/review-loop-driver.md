...
- **dev-solo native Codex reviewer (design and code)**: spawn exactly as `skills/dev-solo/SKILL.md` requires. Pass the repo root and each run artifact's `.harnie/`-relative path; the reviewer reads each one from stdout through `loop.mjs export <repo> <rel>` without `--out`. Do not put an absolute `.harnie` path in a Bash read or create a staging copy.
