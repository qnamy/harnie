...
- **Paths only, not content:** the review-unit directory (`.../review/<unit>/`), the current fix delta's **path** (`delta.patch`), and the previous-round ledger's **path**, when present, plus a short scope/intent summary and the producer's verification report (declared tier, baseline↔post failure-set comparison, items handed to human verification). A missing or unsupported verification report is itself a finding — do not go looking for the evidence yourself.
...
- **Contests** (`loop.md` contest gate): a prompt may carry `CONTEST [ID] reason=<altitude|overengineering> : <grounds>` blocks. Settle each in this response: concede → report that ID `resolved` (open a new non-blocking ID if the concern is still worth recording); insist → keep it `open` and state the concrete mistake scenario in its line. **Never concede a correctness, safety, or unverified-risk finding to a contest** — those are not contestable; insist.
