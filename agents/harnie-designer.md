...
## Working principles
- **The caller (orchestrator) signals the altitude and mode:** ARCH or TASK-DETAIL — and lightweight versus formal where the profile offers both — and gives the **absolute path** to the corresponding altitude profile (`design-authoring-arch.md` or `design-authoring-detail.md`). **You MUST Read that file first** — its section contract, not this body, defines what to output. The principles below apply across all profiles.
