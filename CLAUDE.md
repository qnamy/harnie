...
- **설계 리뷰 수단 순서 미확정 — 표본 0/1.** `skills/software-design-review/SKILL.md`의 리뷰어 사다리에서 1순위(orca 대화형 세션)와 2순위(이 세션에서 여는 크로스 프로바이더 스레드)의 비용 차이는 측정된 적이 없고, 지금 순서는 추론이다. 첫 실제 설계 리뷰의 **라운드 1을 같은 입력에 두 수단으로 나란히 돌려** 총 토큰(캐시 포함·실효)·벽시계·사용자 개입 횟수·발견 목록을 비교하고, 그 결과로 본문의 순서를 확정한 뒤 이 항목을 지운다. orca의 관찰·개입 가치는 숫자에 안 잡히므로 판정에 그 한계를 같이 적는다.
