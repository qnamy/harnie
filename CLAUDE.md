...
- **Codex 무인 모드에서는 harnie run이 돌지 않는다(0.14.2 시점, 미해결).** `captureTree`가 임시 git 인덱스를 `os.tmpdir()`에 만드는데(`scripts/delta.mjs`) `codex -a never` 세션에서는 그 쓰기가 거부되어 `init`은 물론 `runs`까지 `Operation not permitted`로 실패한다. Codex에서 harnie를 돌릴 때는 승인을 켜 두고, 무인 실행이 필요하면 이 제약을 먼저 해소한다(인덱스를 run root 안으로 옮기는 것이 후보). 해소하면 이 항목을 지운다.
