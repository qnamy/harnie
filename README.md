...
`v0.16.10` · 테스트 5 pass (`node --test hooks/*.test.mjs`) · MIT
...
**`dev`는 간단한 작업을 한 지시로 끝까지 돌린다.** 설계 → 설계리뷰 → 구현 → 구현리뷰 → 검증을 순서대로 실행하고, 리뷰와 검증은 다른 프로바이더 세션 하나가 맡는다. 요구사항 단계는 없고 요청 원문이 기준선이다. 시작 전에 크거나 위험한 신호(요구 모호·스키마/마이그레이션·외부 계약·다중 저장소·인증/결제 경로·병렬 요청)가 있으면 거절하고, 설계가 3절 파일 6개를 넘거나 병렬 단위를 담거나 7절에 차단 항목이 있으면 설계를 남기고 넘긴다. 각 단계의 중단은 그 단계에서 끝난다.

**병렬은 설계가 정하고 orca가 실행한다.** 여러 세션이 할 일이면 `software-design`이 3절 끝에 병렬 단위(유닛별 파일 단독 소유·공유 기반 유닛·의존)를 적고 `software-design-review`가 분할 결함을 검사한다. 하위 워크트리 생성·유닛 세션·머지·정리는 `orca-dispatch.md`(agent-ops)의 절차다. 설계가 결정을 끝낸 유닛은 구현 → 구현리뷰를, 경계만 정하고 내부를 남긴 유닛(상세설계 위임)은 `dev`를 돈다. 검증은 전부 머지한 뒤 부모에서 1회 돈다.

**리뷰어는 다른 프로바이더의 별도 세션이다.** 설계나 코드를 쓴 세션이 코디네이터가 되어 orca로 대화형 세션 하나(Claude에서는 Codex, Codex에서는 Claude)를 열고, 요청과 결과를 파일로 주고받는다. 자율 라운드는 3회이고 4라운드부터는 요청 파일 첫 줄에 사용자 해제를 원문으로 인용한다. 한도 보고는 왜 3라운드 안에 닫히지 않았는지를 먼저 답하고, 한 결정이 3라운드 연속 지적을 받으면 고치기 전에 제거를 먼저 평가한다. 발견은 심각도 라벨이 아니라 필요성으로 수용하고, 장치를 추가하는 수정은 삭제·범위 축소·관찰 유예로 닫히는지 먼저 본다. **수용에도 근거를 적는다** — 처분 표의 사유 칸은 모든 처분에 필수이고, `수용`에는 이번 라운드에 원 출처에 대고 확인한 것을 적는다(인용된 행이 실재한다는 확인은 그 칸을 채우지 못한다). 발견이 실재하지만 요구하는 수정이 과하면 `부분 수용`으로 필요한 만큼만 반영하고 나머지는 사유와 함께 기각한다. 검증 세션은 구현을 쓴 세션이 아니어야 한다.
...
**관측되지 않은 수치·조건은 들어가지 않는다.** 기제가 물고 도는 런타임 값 — 타임아웃·재시도 횟수·주기·속도/동시성 상한, 재시도를 거는 에러 코드, 건너뛰기를 거는 상태 — 은 이 시스템의 측정 하나에만 선다. 외부 레퍼런스는 기제를 정당화할 뿐 그 값을 고정하지 못하고, 요구사항이 값을 적었다는 것도 예외가 아니다. 요구사항이 기제 자체를 지목하지 않았는데 설계자가 골랐다면 그 선택부터 외부 레퍼런스나 관측에 선다. 근거는 [docs/design-artifact-references.md](docs/design-artifact-references.md) §21.

**산출물은 워크트리 루트 `_chain/` 하나에 둔다.** 한 워크트리에 한 체인. 커밋하지 않고 `.gitignore`에도 넣지 않는다. 구현 스킬의 clean 트리 확인과 구현리뷰의 변경축은 이 디렉터리만 무시한다. **체인은 검증 파일이 쓰이거나 변경이 머지·릴리스되면 끝난다.** 그 뒤는 새 체인이고 비워진 `_chain/`에서 시작한다 — 다른 체인의 파일이 남아 있으면 스킬이 멈추고 묻는다(지우는 것은 사용자이고 스킬은 삭제하지 않는다). 라운드 번호 `N`은 재시작·회귀를 포함해 체인 전체에서 증가하며 재사용·덮어쓰기·번호 외 이름을 금지한다.

**체인 불변식을 강제하는 훅은 없다.** 리뷰어·검증자가 결과 파일 외를 고치지 않는 것은 스킬 문장과 세션을 여는 실행 옵션이 지키고, 실제 위반이 반복 관측되면 그 지점만 훅으로 올린다. 유일한 훅은 `hooks/skill-guard.mjs`로, Claude Code에서 체인이 대체하는 번들 스킬(`code-review`·별칭 `review`·`simplify`) 호출을 막고 대체 스킬을 안내한다(2026-09-08 headless Claude에서 deny 실측). Codex에는 스킬 호출 훅 이벤트가 없어 발화하지 않으며, Codex 로더가 `UserPromptExpansion` 항목을 어떻게 다루는지는 플러그인 동기화 후 확인 전이다. 근거는 [docs/design-artifact-references.md](docs/design-artifact-references.md) §14.
...
├── skills/           # 개발 체인 6종 + 조합 스킬 dev + 체인 밖 스킬 6종
