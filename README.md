...
> **v0.5.0** · 테스트 253 pass / 0 fail (`node --test scripts/*.test.mjs hooks/*.test.mjs`) · GitHub 공개, 마켓플레이스 설치, 라이브 E2E 검증 완료
...
| **B2 빌드** | `seal`로 권위 스냅샷을 뜨고 Codex 빌더(workspace-write)에 위임한다. 산출 후 `seal-verify`가 빌더의 권위 파일 훼손을 fail-closed로 잡는다 | 코드 변경 + delta |
| **B3 코드 리뷰** | `harnie-reviewer`(Claude, read-only)가 리뷰한다. 전이는 LLM 선언이 아니라 `scripts/loop.mjs`가 fail-closed로 고정한다: `REVIEWING → APPROVED \| REVISING \| STALLED` | `review/<unit>/{ledger.json, state.json, round-N.txt}` |
...
**③ 자율에는 예산 상한이 붙는다.** 태스크별 기본 예산은 wall-clock 30분과 빌더 Codex 호출 15회다. 80%에서 마무리·보고를 경고하고, 100%에서 다음 빌더 호출을 차단한다. 계속하려면 진행 상황과 블로커를 사용자에게 먼저 드러내고 동의 근거를 `--reason`으로 남겨야 예산이 재시작된다.
...
각 run은 별도 git worktree에서 실행되어 세션 간 충돌 없이 병렬로 진행할 수 있다. `dev-full`은 스코프가 겹치지 않는 태스크를 **태스크별 worktree**에서 병렬 빌드한 뒤 순차 merge한다. 각 태스크는 머지 전에 리뷰를 통과해야 한다.
...
harnie의 상당 부분은 harnie 자신의 루프로 만들어졌다. 현재 히스토리 기준 **커밋 55개 중 32개가 AI co-authored**이고, `claude/*` 브랜치에서 **PR 14건**이 리뷰를 거쳐 머지됐다. 도구를 만드는 과정 자체가 그 도구의 E2E 테스트였던 셈이다.
...
- **태스크 하나의 기본 상한은 30분 / Codex 호출 15회다.** 이보다 큰 태스크는 분해 대상이지 예산 연장 대상이 아니다. 연장은 사람이 상황을 확인한 뒤에만 열린다.
- **작은 수정에 `dev-full`은 과하다.** 오타·한 줄 수정까지 설계 리뷰와 승인 게이트를 태울 이유가 없다. 라우터가 `dev-quick`으로 보내는 이유다.
- **Codex 빌더는 `.harnie/`를 읽지 않는다.** 그래서 승인된 설계는 프롬프트에 인라인으로 주입하고 어느 리비전에서 왔는지(`rev-N`)를 명시해 귀속을 유지한다.
- **워크스페이스 run은 직속 하위 repo만 본다.** 임의 깊이의 중첩 구조는 대상이 아니다.
- **resume은 디스크 상태 재도출 기준이다.** 살아 있는 세션의 연속성은 최적화일 뿐 보장 대상이 아니다.
