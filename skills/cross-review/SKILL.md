...
- The Bash guard denies any command mentioning `.harnie` that is not a sanctioned CLI invocation — a single plain `node <ROOT>/scripts/{loop,execution}.mjs …` with no shell metacharacters, `<ROOT>` being the loaded plugin's own path. The sanctioned execution subcommands added for run transfer are `runs --root <repo>`, `handoff --root <repo> --slug <slug>`, `rebind-tree --root <repo> --slug <slug> --unit <unit> --files <paths>`, and `abandon --root <repo> --slug <slug> --confirm <slug>`; use `abandon` rather than editing control files when a run must be discarded. Write `round-N.txt` with the Write tool, not with shell redirection; `ledger.json`/`state.json` are control files no tool may write directly. **To read an artifact under `.harnie` through a shell-capable reviewer**, use `loop.mjs export <repo> <rel>` without `--out` for stdout-only access. Add `--out <path outside .harnie>` only when a real staging file is required; that form returns `{src, out, bytes}` as its provenance record. A Claude subagent reviewer needs neither form because its Read tool can open the run path directly. For dev-solo, the orchestrator uses the stdout form and includes the returned content in the native Codex subagent's prompt because the subagent has no separate filesystem Read tool.
