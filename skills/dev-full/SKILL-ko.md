...
>
> **알려진 의존성, 여기서 구현하지 않음 — B2′/B3′에 걸친 세 단계가 이게 풀리기 前엔 설치된 세션에서 완결되지 않는다.** 현재 `scripts/guards.mjs`·`scripts/execution.mjs`·`scripts/worktree.mjs`(별도 작업 소유, 이 스킬은 아래 B1이 이미 다루는 것 이상으로 고치는 범위 밖)를 직접 대조해 확인함 — 그중 하나는 throwaway repo에서 실제로 재현까지 함:
> 1. **Codex 빌더 가드(`decideCodex`)가 B2′ 4단계를 강하게 막는다.** `cwd`가 단일 활성 루트와 정확히 같아야 하는데, 태스크 worktree의 `cwd`는 그 검사를 그대로 통과 못 해 workspace-write Codex 빌더 호출 자체가 거기선 아예 돌 수 없다.
> 2. **태스크 worktree를 대상으로 하는 `loop.mjs delta`/`apply`는 Bash 가드가 거부한다 — 두 루프 다 걸린다(B2′ 3단계 DR `apply`, 5단계 CR `delta`/`apply`).** `guards.mjs`의 sanctioned-CLI 검사는 `loop.mjs`의 repo/`--root` 인자가 단일 활성 루트와 정확히 같을 것을 요구하는데, 태스크 worktree는 그렇지 않아 sanctioned로 인식되지 않는다. `delta`의 `--out`과 `apply`의 `--ledger`/`--state`/`--review`는 필연적으로 `<taskWt>/.harnie/review/…`를 가리키고, 그 안의 리터럴 `.harnie/` 부분 문자열이 가드의 포괄적 Bash `.harnie` 보호에 걸려 거부된다 — namespace와 무관하게. (`.harnie`를 참조하는 플래그가 없는 `loop.mjs capture <taskWt>` 단독 호출은 영향 없고, B2′ 6단계의 `commit` 같은 평범한 git 명령도 명령 텍스트에 `.harnie`가 전혀 없으니 영향 없다 — 막히는 건 좁게 `delta`/`apply`의 출력 경로뿐이다.)
> 3. **태스크 worktree에 `.harnie/` 내용이 남아 있으면 `worktree.mjs remove`가 성공할 수 없다(B3′ 5단계) — 직접 재현함**: `git worktree remove`는 `fatal: '<경로>' contains modified or untracked files, use --force to delete it`로 거부하고, `removeWorktree`는 `--force` 옵션을 CLI 어디에도 노출하지 않은 채 그냥 `git worktree remove <dir>`만 부른다. B2′ 2~3단계와 5단계가 태스크 worktree에 `design/rev-N.md`·`review/{design,code}/…`를 쓰고, 6단계의 커밋은(그 단계 자신의 노트대로) `.harnie/`를 일부러 제외하므로 — 커밋하면 run 브랜치를 오염시키니 더 나쁘다 — 5단계가 돌 때 태스크 worktree는 **반드시** untracked `.harnie/` 내용을 갖고 있다. 미리 지울 방법도 없다: Bash `rm -rf <taskWt>/.harnie`도 갭 2와 똑같은 포괄적 `.harnie` 보호에 걸려 막힌다.
> 4. **빌더 스레드 등록(`registerBuilderAuto`)은 시스템 전체에서 동시 building 중인 태스크를 하나만 추적한다**(`buildingUnboundTasks`가 정확히 하나의 `building`+미바인딩 태스크를 요구해야 threadId를 자동 바인딩). 1~3이 고쳐지더라도 태스크 2개가 동시에 building이면 둘 다 threadId 미등록으로 남는다. 이건 위 가드 검사들과 별개인 엔진 레벨 한계다.
>
> 병렬 경로의 **나머지**는 현재 머지된 가드 계층에 대고 실제로 작동한다 — 추정이 아니라 직접 검증함: B2′ 1~2단계(worktree 생성·경량 설계)는 전부, 3단계는 Codex 설계리뷰 호출 자체는(read-only sandbox는 빌더와 달리 `cwd` 제약이 없다) 되지만 그 `apply`는 안 됨(갭 2), 그리고 6단계(커밋), 그리고 **B3′ 자신의 호출들과 B3 전체** — 이들의 모든 `execution.mjs`/`loop.mjs`/`worktree.mjs` 호출은 `--repo`/`--root`가 run worktree(단일 활성 루트)와 같아서 가드가 sanction한다. 그 안에서 실패하는 유일한 호출은 B3′ 5단계(`worktree.mjs remove`)이고, 그건 가드 거부가 아니라 갭 3의 이유 때문이다. 이 스킬은 위 네 가지에 대해 어떤 우회도 시도하지 않는다 — 훅을 끄거나 `guards.mjs`/`execution.mjs`/`worktree.mjs`를 고치거나 `.harnie/`를 손으로 지우는 것은 가드를 만족시키거나 상태를 보호하는 게 아니라 무력화·훼손하는 것이다. 위 어느 단계든 실패하거나 거부되면, 우회하지 말고 STOP하고 보고한다. **이건 웨이브-2 통합 항목이지, 조용히 우회할 거리가 아니다** — 이 병렬-개발 노력의 조각들(가드/엔진, `worktree.mjs`, 이 스킬)이 함께 돌려지는 자리에서 명시적으로 제기한다. 어느 한 조각도 단독으로 고칠 수 있는 게 아니기 때문이다. `worktree.mjs` 소유자가 `remove`에 `--force` passthrough나 `.harnie`-인지 자동 정리를 추가하는 게 자연스럽다(`create`의 exclude 등록에서 이미 비슷한 `.harnie`-인지 뒷정리를 하고 있으므로). 갭 1·2는 hook 레벨의 강한 deny(`deny: true`)이지 사용자가 승인해서 넘어갈 수 있는 권한 프롬프트가 아니고, 갭 3은 CLI에 노출된 override가 없는 평범한 git 거부다. 어느 것도 오늘 살아있는 우회로가 없다.
...
5. `node <ROOT>/scripts/worktree.mjs remove --repo <repo> --branch harnie/<slug>-t<id>` — `--delete-branch` 플래그 없이(기본이 이미 브랜치를 유지한다). **이건 현재 실패한다** — 태스크 worktree에 `.harnie/` 내용이 남아 있는 한(이 스킬 자신의 설계상 모든 태스크 worktree가 그렇다 — 2~3단계가 `design/rev-N.md`·`review/design/…`를, 5단계가 `review/code/…`를 쓰고 6단계는 절대 커밋하지 않으므로). `git worktree remove`는 untracked 파일이 있는 worktree의 제거를 거부하고, `worktree.mjs remove`는 `--force`를 노출하지 않는다 — 위 알려진 의존성 참조. 실패하면 손으로 `.harnie/`를 지우거나(delta/apply와 같은 이유로 Bash `rm -rf`가 막힌다) 지원 안 하는 플래그를 넘기지 말고, STOP하고 보고한다.
