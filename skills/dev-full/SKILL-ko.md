...
>
> **알려진 의존성, 여기서 구현하지 않음 — B2′의 두 단계는 이게 풀리기 前엔 설치된 세션에서 돌지 않는다.** 현재 `scripts/guards.mjs`·`scripts/execution.mjs`(별도 작업 소유, 이 스킬은 아래 B1이 이미 다루는 것 이상으로 고치는 범위 밖)를 직접 대조해 확인함:
...
> 2. **태스크 worktree를 대상으로 하는 `loop.mjs delta`/`apply`(B2′ 5단계)는 Bash 가드가 거부한다.** `guards.mjs`의 sanctioned-CLI 검사는 `loop.mjs`의 repo/`--root` 인자가 단일 활성 루트와 정확히 같을 것을 요구하는데, 태스크 worktree는 그렇지 않아 sanctioned로 인식되지 않는다. `delta`의 `--out`과 `apply`의 `--ledger`/`--state`/`--review`는 필연적으로 `<taskWt>/.harnie/review/…`를 가리키고, 그 안의 리터럴 `.harnie/` 부분 문자열이 가드의 포괄적 Bash `.harnie` 보호에 걸려 거부된다. (`.harnie`를 참조하는 플래그가 없는 `loop.mjs capture <taskWt>` 단독 호출은 영향 없고, B2′ 6단계의 `commit` 같은 평범한 git 명령도 명령 텍스트에 `.harnie`가 전혀 없으니 영향 없다 — 막히는 건 좁게 `delta`/`apply`의 출력 경로뿐이다.)
> 3. **빌더 스레드 등록(`registerBuilderAuto`)은 시스템 전체에서 동시 building 중인 태스크를 하나만 추적한다**(`buildingUnboundTasks`가 정확히 하나의 `building`+미바인딩 태스크를 요구해야 threadId를 자동 바인딩). 1·2가 고쳐지더라도 태스크 2개가 동시에 building이면 둘 다 threadId 미등록으로 남는다. 이건 위 두 가드 검사와 별개인 엔진 레벨 한계다.
>
> 병렬 경로의 **나머지**는 현재 머지된 가드 계층에 대고 실제로 작동한다 — 추정이 아니라 직접 검증함: B2′ 1~3단계(worktree 생성·경량 설계·Codex 설계리뷰)와 6단계(커밋), 그리고 **B3′·B3 전체** — 이들의 모든 `execution.mjs`/`loop.mjs`/`worktree.mjs` 호출은 `--repo`/`--root`가 run worktree(단일 활성 루트)와 같아서 가드가 sanction한다. 막히는 건 오직 B2′ 4·5단계뿐이다. 이 스킬은 위 세 가지에 대해 어떤 우회도 시도하지 않는다 — 훅을 끄거나 `guards.mjs`/`execution.mjs`를 고치거나 상태 파일을 손으로 써서 deny를 피하는 것은 가드를 만족시키는 게 아니라 무력화하는 것이다. 훅이 어느 단계를 거부하면, 우회하지 말고 STOP하고 보고한다. (별개의 좁은 참고: `guards.mjs`의 Write/Edit control-path 검사 `isControlPath`는 `.harnie/`로 시작하는 경로만 인식하므로, `.harnie-wt/` 아래 중첩된 태스크 worktree 자신의 `.harnie/review/…`는 run worktree의 것만큼 그 검사로 독립 보호되지 않는다 — 어느 쪽이든 손으로 고치지 않는다, 이건 가드의 보장이 아니라 위임 참조 규칙의 규율이다.) **이건 웨이브-2 통합 항목이지, 조용히 우회할 거리가 아니다** — 이 병렬-개발 노력의 조각들(가드/엔진, `worktree.mjs`, 이 스킬)이 함께 돌려지는 자리에서 명시적으로 제기한다. 어느 한 조각도 단독으로 고칠 수 있는 게 아니기 때문이다. 둘 다 hook 레벨의 강한 deny(`deny: true`)이지 사용자가 승인해서 넘어갈 수 있는 권한 프롬프트가 아니다 — 가드/엔진이 태스크 worktree를 두 번째 정당한 루트로 인식하기 전까지는 살아있는 우회로가 없다.
...
5. `node <ROOT>/scripts/worktree.mjs remove --repo <repo> --branch harnie/<slug>-t<id>` — `--delete-branch` 플래그 없이(기본이 이미 브랜치를 유지한다; `--delete-branch`를 넘기지 않는다 — merge된 이력은 어차피 `harnie/<slug>`에서 도달 가능하지만, 여기서 태스크 브랜치를 지울 이유는 없다).
