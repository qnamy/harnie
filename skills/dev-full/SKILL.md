...
> **Model assignment** for every delegation in this run follows `${CLAUDE_PLUGIN_ROOT}/instructions/model-matrix.md`: the **run difficulty** (easy/medium/hard — announced by the `/harnie:dev` router, or judged at A0 on direct entry) tiers **producer** models (Codex builder, `harnie-designer` at the DETAIL altitude) and, conservatively, the **code reviewer** (`harnie-reviewer` — unit reviews: easy = sonnet, medium/hard = opus; confirmation reviews: sonnet below hard; Final Wave gates: always opus); design review stays fixed at `gpt-5.6-sol`; A3 formal architecture design always uses **fable** (fallback opus). Phase files restate the concrete values at each call site.
...
Every path you hand to a subagent or reviewer must be a **disk artifact of record** inside the repo — `.harnie/plan/<slug>/…` or a source file. **Never pass a tool-result blob path** (`tool-results/*.json`, transcript scratch, temp captures): oversized blobs silently fail to load and the delegate reconstructs from a stale revision — a generation mix that is invisible downstream.

1. The current design lives at `.harnie/plan/<slug>/design/rev-N.md` — a **new file per revision, never overwritten**. **The designer writes it there directly** (the delegation names the exact destination; the designer returns only a short summary; main verifies the file exists non-empty before any delegation referencing it). Main never transcribes design text from an agent response into the file. The first authoring delegation writes `rev-1` from the task and its grounding.
2. Every delegation referencing an existing design names **that exact path and revision**, and states which revision is under review.
3. Inlining design content (required by `review-loop-driver.md` R2 for `DR` loops) does not replace this: the inlined content and the named `rev-N.md` must be the **same revision**.
4. **Builder exception (B2).** The Codex builder never receives a `.harnie` path: give it the approved design **inlined** with its `rev-N` named. Path-naming binds delegates that may read `.harnie/` — the designer and design reviewer.

If a delegate reports an unreadable referenced path, its output is **void for that round** — fix the reference and re-delegate. Never accept a result reconstructed from memory.

## Execution State and Mandatory Hooks (Plan-Only Harness: `scripts/execution.mjs`)

The plan track mechanizes two invariants — **① no source writes before approval, ② no declaration of done while unapproved or incomplete** — through durable state and mandatory hooks. Authority = the planHash-bound `manifest.json` (mutable only via A5.2 user re-approval, prior version archived) + each review unit's ledger/state + verification receipts; `execution.json` is an advisory cache, never trusted. `<ROOT>` = `${CLAUDE_PLUGIN_ROOT}`; `<repo>` = this run's absolute **workroot** from the bootstrap hook's context message (fallback: the `workroot` field in `<main repo>/.harnie/sessions/<this session's id>.json`) — not the session's start directory. **Every `execution.mjs` subcommand and `loop.mjs apply` require `--root <repo>`** (`loop.mjs capture`/`delta` take `<repo>` positionally). Manipulate state **only through `execution.mjs`** — hooks block direct writes, and the Bash guard blanket-blocks any command text referencing `.harnie`, **reads included**. To export a `.harnie` artifact use `node <ROOT>/scripts/loop.mjs export <repo> <path relative to .harnie/> [--out <dest outside .harnie>]`; for in-context reading, the Read tool works as always.
...
Measured runs show the dominant token cost is the orchestrator's own accumulated context re-read on **every** call. Three standing rules:

1. **Session split at unit boundaries.** All run authority is durable on disk, so a fresh session resumes losslessly — a long run is *expected* to span several sessions. After roughly 3–4 completed review units in one session (or earlier when large artifacts passed through context), append a short handoff entry to `notepad.md` (current unit, next step, open blockers) and **propose a session split** — the user decides. `loop.mjs apply` backstops this mechanically (`sessionSplitRecommended: true` on every 4th completed unit) — when it fires, propose before starting the next unit; never dismiss it because the session "feels fine".
2. **Injection discipline.** Keep bulk out of main context: read only the sections of `plan.md`/`design/rev-N.md` a decision actually needs (delegates read their own references from the paths you pass, per the Delegation Reference Rule); run large-output commands filtered at the source; never carry a delegate's full report where its verdict line suffices. Run your own searches as single relative-path `rg` commands from the workroot, not the Grep tool — Grep results prefix every output line with the absolute worktree path, and that bulk re-enters context on every later step (Bash-less subagents and `.harnie` paths keep Grep/Read as their sanctioned readers).
