...
**A4. 상세 설계(정식) + 리뷰 루프.** 승인된 아키(또는 기존 아키) 위에서 `harnie-designer`(모델은 `model-matrix.md` §3의 run 난이도에 따름: easy/medium = sonnet, hard = opus — 지원되는 경우 **Task 호출의 모델 오버라이드**로 전달하고, 아니면 frontmatter 기본값 opus)에게 **상세 설계를 "정식으로"** 요청한다 — 위임 프롬프트에 `${CLAUDE_PLUGIN_ROOT}/instructions/design-authoring-detail.md`의 **절대경로를 전달**하고 `detailed design, formal`을 신호한다; designer의 에이전트 본문이 그 프로필을 먼저 Read하도록 요구한다. 또한 `plan.md` 경로와 그 `## Environment Fact Sheet` 섹션명을 함께 전달해, designer가 A1의 검증된 사실에서 출발하게 한다(재도출·추정 금지). 요구 추적표·핵심 처리 로직·계약·데이터/상태·작업 분해, decision-complete 수준을 요구한다. 아키 결정을 조용히 바꾸지 않는다(바꿔야 하면 A3로 되돌려 아키 변경 요청). main은 받은 설계를 그것을 참조하는 위임 前에 다음 `.harnie/plan/<slug>/design/rev-N.md`에 쓰고, 같은 리비전을 `plan.md`의 상세 섹션에 기록한다.
...
- **엔진이 이 게이트를 백스톱하지만, 고쳐 주지는 못한다.** `verify`는 각 항목의 출력을 캡처해 껍데기-성공 판정을 돌린다([execution.mjs:138](scripts/execution.mjs:138)): exitCode 0인데 출력 0바이트, `node --test` 통과 테스트 0건, 검색 명령 매치 0건이면 receipt에 `vacuous`가 찍히고, 완료 재도출이 vacuous receipt를 blocker로 거부한다([execution.mjs:173](scripts/execution.mjs:173)). 다만 백스톱은 껍데기 항목을 하드 블록으로 바꿔줄 뿐 복구 경로가 아니다: 승인 후 manifest는 immutable이고 `set-phase`는 `planning` 역전이를 거부하므로, 껍데기 항목은 여전히 **그 run 안에서 고칠 수 없다** — 정직한 결말은 그 항목을 지목해 `HARNIE_STATUS: INCOMPLETE`로 보고하고 판단을 사용자에게 넘기는 것뿐이다. 여기서 맞게 잡아라.
