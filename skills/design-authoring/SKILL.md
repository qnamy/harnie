...
1. **Pick the altitude.** Use the ARCH/DETAIL altitude definitions in `${CLAUDE_PLUGIN_ROOT}/instructions/model-matrix.md` §1. When the request is ambiguous, confirm the altitude with the requester once.
...
- The `/harnie:dev-full` and `/harnie:dev-quick` loops load these same contracts directly (phase A / Step 3); do not invoke this skill inside them.
