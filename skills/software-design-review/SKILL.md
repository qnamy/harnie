...
**The response file.** One line per finding raised so far, in id order, four fields: id · 처분 (`수용`/`부분 수용`/`기각`) · 사유 (required for `기각` and for `부분 수용`, where it states what was not taken and why; for a `수용` whose fix adds a mechanism, the requirements sentence or the requester's words that need it — a `수용` that cannot quote one becomes `기각`) · 반영 위치 (design section numbers for `수용` and `부분 수용`; `-` for `기각`). Use `부분 수용` when the finding names something real but the fix it asks for goes past it: apply what the defect needs and reject the rest with its reason. A `수용` or `부분 수용` without 반영 위치 is unfinished.

**After applying, check the design yourself**: section 6 still reaches the change; every requirements `[미결정]` is still there; when every disposition this round was `수용`, each finding's failure scenario held against the design as it stood before the fix — an all-accepted round is itself the signal to check; and **the design's mechanisms have not grown past round 1 without a requirement naming why**. That growth is yours to reverse, not a reviewer finding.
