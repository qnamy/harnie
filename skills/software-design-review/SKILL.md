...
- **Fact error** — an environment fact that is false, or unsupported and not marked open; a runtime number or condition a mechanism turns on — a timeout, retry count, interval, rate or concurrency limit, the error code or state it branches on — is such a fact unless a measurement of this system stands behind it, the requirements naming the value themselves being no exception. Report every one in the same round.
...
**Result.** Korean: one verdict line, findings ordered by severity, then questions. From round 2 on, report every earlier finding as `open` or `resolved`, resolved only where you verified it in the current design — omission is not resolution. A finding the coordinator rejected with a reason closes if the reason holds; otherwise it stays open, naming which of the reasons fails and why, with its failure restated. A `부분 수용` closes only when the defect the accepted part named is gone from the current design and the reason given for the rejected part holds; whichever of the two fails stays open, and both do when both fail. Re-review covers the open findings and the revision delta.
...
- **Three rounds on your own.** At that limit with findings still open, stop and report them to the user, and that report answers why three rounds did not close them — name the decisions the rounds circled — before a release is asked for. A round the user releases has the same scope as any re-review. A released round's request opens with the user's release quoted in their words; a decision the user made on a `discuss:` item is not a release.
...
**Accepting.** By necessity, never by label. Accept what prevents a concrete failure inside the requirements, names a real defect, or is cheap with clear value. Reject what adds a mechanism with no named failure, expands scope past the requirements, or is taste. Check a rejection reason against the boundaries and decisions the design itself already set before you write it; where the reason and that boundary disagree, the boundary is right. **Before accepting a fix that adds a runtime mechanism — state, retry, lock, controller, health check, boot service — first check whether removing something, narrowing scope, or leaving it to pilot observation closes the finding. When a finding arose from the previous round's fix, evaluate reverting that fix first. When one decision — a verification step included — has drawn a finding in three rounds running, evaluate removing that decision before any fix and record that comparison in its 사유.** This three-round removal evaluation is scoped to design contracts — `implementation-review` intentionally does not carry it. `discuss:` is not yours to settle: carry it into section 7 with the decider it names and raise it to the user.

**The response file.** One line per finding raised so far, in id order, four fields: id · 처분 (`수용`/`부분 수용`/`기각`) · 사유 · 반영 위치 (design section numbers for `수용` and `부분 수용`; `-` for `기각`). **사유 is required on every disposition**, and what it holds differs by one.

| 처분 | 사유 holds |
|---|---|
| `수용` | What you checked against the original source this round to confirm the finding's failure scenario held in the design as it stood before the fix — the `file:line` you opened, the command you ran and its result, the requirements sentence you compared. Confirming the quoted line exists is not that check. When the fix adds a mechanism, add the requirements sentence or the requester's words that need it; a `수용` that cannot quote one becomes `기각` |
| `부분 수용` | The same check for the part taken, plus what was not taken and why |
| `기각` | The reason |

A `수용` whose 사유 you cannot fill is unfinished: do the check and fill it, or it becomes `기각`. Use `부분 수용` when the finding names something real but the fix it asks for goes past it: apply what the defect needs and reject the rest with its reason. A `수용` or `부분 수용` without 반영 위치 is unfinished.

**After applying, check the design yourself**: section 6 still reaches the change; every requirements `[미결정]` is still there; **every environment fact this round's fixes newly assert carries the observation it was verified against, quoted as it reads** — the repository path for a fact inside this repository, the source's own words for a fact outside it, and for a live one the command and its output, the log line, or the record's actual field value — but a runtime number or condition a mechanism turns on takes the measurement its own rule demands, an external source never fixing one, never your diagnosis of it and never a record identifier with a categorical summary, an unquoted assertion not being applied at all; when every disposition this round was `수용`, each finding's failure scenario held against the design as it stood before the fix — an all-accepted round is itself the signal to check; and **the design's mechanisms have not grown past round 1 without a requirement naming why**. That growth is yours to reverse, not a reviewer finding.
