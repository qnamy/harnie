...
**After applying, check the design yourself**: section 6 still reaches the change; every requirements `[미결정]` is still there; **every environment fact this round's fixes newly assert carries the observation it was verified against, quoted as it reads** — the repository path for a fact inside this repository, and for a live one the command and its output, the log line, or the record's actual field value, never your diagnosis of it and never a record identifier with a categorical summary, an unquoted assertion not being applied at all; when every disposition this round was `수용`, each finding's failure scenario held against the design as it stood before the fix — an all-accepted round is itself the signal to check; and **the design's mechanisms have not grown past round 1 without a requirement naming why**. That growth is yours to reverse, not a reviewer finding.
