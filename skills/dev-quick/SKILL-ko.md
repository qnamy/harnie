...
### 1. Intent & size
작업을 한 줄로 재진술한다. **진짜 작은가**(새 컴포넌트·경계/계약 변경·아키텍처 결정 없음) 확인 — 이것들이 ARCH-altitude 트리거이며, quick은 DETAIL 고도만 지원한다(`model-matrix.md` §1). 크면 멈추고 `/harnie:dev-full`을 권한다.

그다음 **run 난이도**를 고정한다: `/harnie:dev` 라우터가 이미 easy/medium/hard를 announce했다면 계승하고, 직접 진입이면 `model-matrix.md` §2의 rubric으로 지금 판정해 announce한다. run당 1회 판정 후 고정하며, Step 3~4의 **생산자 모델**과 코드 리뷰어 티어를 선택한다(`model-matrix.md` §3).

### 2. Read (필요시)
낯선 코드면 `harnie-scout`(haiku)를 병렬 스폰해 관련 위치를 잡는다. 자명하면 skip.

### 3. (옵션) 상세 설계(경량) + 설계 리뷰
비자명하면 **상세 설계를 경량 모드로** 산출한다(producer = **Claude** `harnie-designer`). 작성 계약 = `${CLAUDE_PLUGIN_ROOT}/instructions/design-authoring-detail.md` 경량. 작으면 main 직접, 비자명하면 `harnie-designer` 위임(모델은 `model-matrix.md` §3의 run 난이도에 따른다: easy/medium = sonnet, hard = opus — 지원되는 경우 **Task 호출의 모델 오버라이드**로 전달하고, 아니면 frontmatter 기본값 opus; 이때 **그 파일의 절대경로를 제공**하고 "상세 설계, 경량"을 신호; designer의 에이전트 본문이 작성 前 프로필을 Read하도록 정해져 있으므로 프롬프트에 내용을 붙여넣지 말 것). quick은 구조상 **상세(DETAIL) 고도만** — 새 컴포넌트·경계 변경·아키텍처 결정이 필요하면 애초에 `/harnie:dev-full`이다(Step 1에서 걸러짐). "정식으로"를 붙이지 않는다(경량 기본, 깊이 차등으로 작은 작업엔 몇 줄로 수렴).
**설계는 `.harnie/quick/<slug>/review/design/design.md`에 있어야 한다**(Step 4 개발·리뷰가 읽는 단일 소스) — `harnie-designer`에게 위임할 때는 그 경로를 목적지로 지명하고 designer가 직접 쓴다(main은 파일 존재만 확인); 소규모 작업을 main이 인라인으로 작성할 때는 main이 쓴다. 그다음 → **설계 리뷰 루프**(리뷰어 = **Codex** (`model:"gpt-5.6-sol"`, 고정 — `model-matrix.md` §3 참조), design-review.md 기준, 상세 고도, ID namespace `DR`, `<dir>` = `.harnie/quick/<slug>/review/design/`)를 review-loop-driver.md대로 APPROVE까지 돌린 뒤 코드로 넘어간다. **설계 루프는 R1 git-delta를 쓰지 않는다**(`design.md`는 `.harnie/` 제외 대상 → delta 항상 빈값). 대신 리뷰어 프롬프트에 **`design.md`의 절대경로를 전달**하되 먼저 읽으라는 지시를 함께(첫 리뷰=경로만, 재리뷰=경로 + 수정된 섹션 이름 목록)하고 R2~R5만 구동한다. 자명하면 Step 3 전체 skip.
