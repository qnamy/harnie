...
그다음 **run 난이도**를 고정한다: `/harnie:dev` 라우터가 이미 easy/medium/hard를 announce했다면 계승하고, 직접 진입이면 `model-matrix.md` §2의 rubric으로 지금 판정해 announce한다. run당 1회 판정 후 고정하며, Step 3~4의 **생산자 모델**을 선택한다(**리뷰어 모델은 절대 티어링하지 않는다**).
