...
**rev-7** (0.12.0 반영 — U-3 종결, U-1 이행):
- **U-3 종결**: CR-217(rev-6)이 "dev-solo 설계리뷰 모델은 opus 잠정 — 첫 사용 실측 후 확정"으로 열어 둔 open item을 메커니즘 자체 변경으로 닫는다. 0.12.0에서 dev-solo의 `claude -p` 크로스모델 리뷰 경로를 완전히 제거하고 fresh `codex exec --sandbox read-only -m gpt-5.6-sol` 셀프리뷰만 남겼다(설계 리뷰·코드 리뷰 모두 동일 경로) — 그 결과 "opus vs sol 중 무엇을 쓸지" 비교할 대상 자체가 사라졌으므로, opus-잠정 값을 실측으로 확정하는 절차가 더는 의미가 없다. U-3는 **실측 없이, 메커니즘 변경으로 종결**한다.
- **U-1 이행**: §1 파일 인벤토리(L13)가 "1버전 유지"로 약속한 alias 삭제를 0.12.0에서 실제로 이행한다 — `skills/dev-quick/SKILL.md`·`SKILL-ko.md`·`skills/dev-full/SKILL.md`·`SKILL-ko.md` 4파일을 완전히 삭제했다. `.claude-plugin/plugin.json`은 `commands`·`agents`만 목록화하고 `skills`는 목록화하지 않으므로(디렉터리 존재로 자동 인식) 추가 편집이 필요 없음을 확인했다. 삭제 후 repo 전체 grep(`dev-quick|dev-full`)으로 남은 참조는, ① README·design-0.10-restructure.md 등의 과거형 프로젝트 히스토리 서술("그때 그런 일이 있었다" — "지금 alias가 존재한다"는 현재형 주장이 아니다), ② `docs/architecture.md` §4·§6의 0.10 트랙 구조 기록(이번에 "0.10 이력" 표시를 명시적으로 추가했다 — §6.4(c) 참고), ③ `hooks/bootstrap.mjs`·`pretooluse.mjs`·`stop.mjs`에 이번에 정확한 도달 가능성으로 다시 쓴 주석(아래 세 번째 항목 참고). **정정(DR-004, 3차 리뷰까지 반영)**: "실행 가능한 진입점으로서의 `dev-quick`/`dev-full` 참조는 0건", "PreToolUse(Skill) 경로만 실제로 도달 불가"라는 앞선 두 차례의 주장은 모두 과도한 확신이었다 — 정확한 사실은: `hooks/bootstrap.mjs`의 어떤 분기도 스킬 파일 존재를 검사하지 않는다. `UserPromptSubmit` 직접 슬래시 매치는 **사용자가** 여전히 타이핑해 도달할 수 있지만 둘의 동작은 다르다 — `/harnie:dev-full`(L140)은 부트스트랩까지는 성공하고 이어질 스킬 본문만 없다; `/harnie:dev-quick`(L146)은 원래부터 `doBootstrap`을 호출하지 않고 `clearPendingRoute`만 수행하므로 부트스트랩 자체가 없다(`hooks/bootstrap.test.mjs`의 `active(root) === null` 단언과 일치 — §2.7 참고). `PreToolUse(Skill)` 매치(L158, L163)는 **정상적인 스킬 디스패치**(Claude Code가 실재하는 스킬 이름으로만 `Skill` 도구를 호출)로는 더 이상 자연 발생하지 않지만, 그 payload가 어떤 식으로든 직접 전달되면(예: `hooks/bootstrap.test.mjs`의 테스트 픽스처가 정확히 이렇게 한다 — 이 파일은 스코프 밖이라 손대지 않는다) 훅은 여전히 동일하게 처리한다 — "코드가 스킬 존재를 검사해서 거부한다"는 뜻으로 "dead"를 쓰면 안 된다. 이 저장소 전체 grep(`dev-quick|dev-full`) 결과, 36파일 스코프 밖에 남는 참조는 다섯 범주다: ① README·design-0.10-restructure.md 등 과거형 프로젝트 히스토리, ② `docs/architecture.md` §4·§6의 "0.10 이력" 표시(이번에 추가), ③ `hooks/*.mjs`에 남는 실제 분기 코드 자체(정규식·매치·`clearPendingRoute`/`doBootstrap` 호출)와 이번에 정정한 주석 — 분기는 로직 수준에서 계속 살아있고, 이번 run은 그 도달 가능성을 정확히 서술하는 주석만 얹는다(§2.7), ④ `hooks/bootstrap.test.mjs`·`hooks/hooks.test.mjs`·`hooks/fixtures/bootstrap-hook-events.json`(0.12.1로 이관되는 테스트 재작업 대상 — plan.md rev-4가 명시적으로 스코프 밖으로 뺀 파일들)와 `pretooluse.mjs`·`stop.mjs`의 안내 문자열 본문(로직·문구 모두 무변경, 0.12.1로 이관), ⑤ `hooks/lib.mjs` L54("bootstrap이 dev-full...")와 `scripts/loop.mjs` L247("dev-full SKILL의 세션분할 제안")의 부수적 주석 언급 — 둘 다 36파일 스코프 밖이며 로직에 영향 없는 설명 주석이라 이번 run에서는 건드리지 않는다(발견 사실만 기록).
- **hooks 분기는 이번 범위 밖**: `hooks/bootstrap.mjs`·`pretooluse.mjs`·`stop.mjs`의 dev-full/dev-quick 관련 분기 중 어느 쪽도 코드 수준에서 "도달 불가능"해지지 않는다 — `PreToolUse(Skill)` 매치(L158, L163)는 정상적인 스킬 디스패치에서는 더 이상 자연 발생하지 않지만(호출할 스킬 이름 자체가 없으므로), 그 payload가 직접 전달되면(`hooks/bootstrap.test.mjs`가 정확히 이렇게 한다) 훅은 여전히 동일하게 처리한다(스킬 존재를 검사하지 않음). `UserPromptSubmit` 직접 슬래시 경로(`/harnie:dev-full`·`/harnie:dev-quick` 직접 타이핑, L140·L146)는 사용자가 실제로 타이핑해 도달할 수 있다는 점에서 자연 발생 경로 자체가 살아있다(`writePendingRoute`의 프로덕션 호출은 여전히 0건). 로직 자체의 제거(두 경로 모두)와 그에 딸린 `hooks/bootstrap.test.mjs`·`hooks/hooks.test.mjs`·`hooks/fixtures/bootstrap-hook-events.json`의 ~15개 테스트 재작업(단순 문자열 치환이 아니라 호출 시퀀스 재구성 필요), 그리고 안내 문자열(`pretooluse.mjs`·`stop.mjs`의 `denyPreTool`/`blockStop` 메시지 본문)의 정정은 사용자가 명시적으로 0.12.1 후속 run으로 분리했다 — 이번 0.12.0은 주석을 정확한 도달 가능성으로 다시 쓰는 것까지만 하고 로직·정규식·분기 순서·안내 문자열은 그대로 둔다.
