...
## 22. 항상 주입 지침의 중복 서술이 만든 소유 불명시 (2026-09-18)

전역지침 전체 리뷰에서 두 리뷰 스킬의 §Coordinator·§Both roles, never가 약 80% 동일 문장의 수동 복제이고, repo AGENTS.md가 그 상세를 다시 서술하는 것을 확인했다. 불일치는 이미 있었다 — "한 결정이 3라운드 연속 지적을 받으면 제거를 먼저 평가"는 `software-design-review`에만 있고(§20 사건이 설계리뷰의 것이라), 환경 사실 두 질문과 `설계 회귀` 처분은 `implementation-review`에만 있는데 AGENTS.md는 둘을 체인 공통 규칙처럼 서술해 어느 쪽 소유인지 읽히지 않았다. 같은 내용이 Devin에서 worktree AGENTS.md와 플러그인 캐시 AGENTS.md 양쪽으로 트리거되는 것도 이 리뷰 세션에서 관측했다.

| 관측 | 조치 | 어디에 |
|---|---|---|
| AGENTS.md가 스킬 계약 상세(처분 표 사유 필드·환경 사실 두 질문·제거 평가·`_chain/` 상세)를 재서술해, 스킬과 어긋나도 어느 쪽이 정본인지 구분되지 않는다 | AGENTS.md는 라우팅만 둔다 — 별도 세션·orca·파일 교환·자율 라운드 3·`_chain/` 위치만. 상세는 소유 스킬명으로 위임하고 "두 리뷰 스킬의 규칙은 같지 않다"를 붙인다 | repo `AGENTS.md` 둘째 단락 |
| 복제된 프로토콜에서 의도된 차이가 문서상 구분되지 않는다 | 각 스킬이 상대에 없는 규칙에 소유 스코프 한 줄을 단다(상대 스킬명과 없는 이유) | `software-design-review` §Accepting · `implementation-review` §Coordinator |
| 상주 규칙에 측정값·사건 서사·날짜가 섞여 매 세션 지불된다(전역 16.8KB, repo 10.5KB) | 규칙 문장·기제·라우팅은 남기고 측정과 사건을 뺀다. 라우팅 대상이 있는 근거는 그 문서가 이미 담고, 대상이 없는 측정은 git 히스토리가 보관한다 | 전역 `AGENTS.md`(agent-ops 정본) |

| 후보 | 기각 사유 |
|---|---|
| 공유 코디네이터 프로토콜 파일을 빼고 두 리뷰 스킬이 참조한다 | 이식 규약이 스킬 루트 기준 상대경로 한 단계만 허용해 스킬 디렉터리 밖의 공유 파일을 가리키지 못한다. 소유 명시가 같은 결함(소유 불명시)을 구조 변경 없이 막는다 |
| 설계 문서 절 번호 참조(5개 스킬 + README + AGENTS)를 절 이름으로 바꾼다 | 번호 드리프트 사고가 관측되지 않았고 참조 일괄 개명의 churn이 막을 실패보다 크다 |
| 지침 규칙에 퇴역 절차를 둔다 | 규칙 관리 기제 신설이 이 문서가 막으려는 오버엔지니어링과 같은 형태다. 사건→규칙 원장(이 파일)과 git 히스토리가 그 역할을 이미 한다 |
| 15분 라운드 한도·400줄 그루핑에 측정을 요구한다 | 측정 규칙은 설계되는 소프트웨어의 런타임 값을 묶는 규칙이다. 이 값들은 "unless the user says otherwise"·"roughly"로 이미 기본값 프레이밍되어 있다 |
