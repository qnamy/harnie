...
> 최종 갱신: 2026-08-13 (`loop.mjs` **symlink 재지정 거부 계층 철회**(§6 (39)·(40) supersede — 설계 §0.1 비목표) + `loop.md`/`loop-ko.md`에 **severity 고정 규칙**(ID 수명 동안 심각도 불변, resolved도 원래 심각도로 emit). 테스트 **171 pass** 유지. 직전 2026-08-12: **adherence 갭 구조적 해결 = 종료**: 진입점 재편(`dev`/`dev-full`/`dev-quick`) + `UserPromptSubmit`+`PreToolUse Skill` bootstrap + pending-route 게이트 — 구현·코드리뷰 6R(APPROVE)·**테스트 171 pass**·**설치본 라이브 FULL E2E PASS**(자연 실행, 디스크 권위 독립 재검증)·`origin/main` 푸시 완료. dev-full A1/A2 조사·질문 계약 강화 + `decideTask` plugin-namespace 버그 수정 포함. **다음 = ⑦ 잔여(②③·포트폴리오 서사)**. 상세 = ADR `docs/bootstrap-adherence.md` + §7 핸드오프. (`UserPromptExpansion`은 2.1.90 미지원 → `UserPromptSubmit`.)
>
