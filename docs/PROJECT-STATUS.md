...
- **⑦ 배포:** 개인 GitHub push(폴더 `harnie`) → 설치 라이브 검증 → ②③(REVIEW.md 은퇴·루틴 rewire) → 포트폴리오 서사.
