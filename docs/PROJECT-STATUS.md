...
> 최종 갱신: 2026-08-10 (C 구현 APPROVED + `origin/main` 푸시 완료 — §7 핸드오프 참조).
>
...
- `agents/{harnie-scout,harnie-designer,harnie-builder,harnie-reviewer}.md` — 역할 에이전트(harnie-reviewer=read-only 코드 리뷰어, ⑥.6 C)
...
- `hooks/{hooks.json,lib.mjs,pretooluse,stop,posttooluse}.mjs` — H1 승인前·control 가드 / H2 미완료-확정 방지(HARNIE_STATUS footer) / PostToolUse 관찰 등록. 테스트 전체 115 pass
...
**⑥.6 = v0 필수(⑦ 前 게이트, codex 협의로 확정) — 순서 A → C → B.** 현재 위치: **A ✅ 완료 · C 설계 ✅ APPROVED(rev.10) · C 구현 ✅ APPROVED(Codex 코드리뷰 12라운드, blocker 없음, 테스트 115 pass) → `origin/main` 푸시 완료(`9fcf719`, `github.com:qnamy/harnie`) · 다음 실행 = 「B(스왑 full-loop E2E)」** → ⑦.
...
- **B (P0, 게이트):** ⬜ **스왑 full-loop E2E** — Codex 구현→delta 귀속→Claude REJECT→codex-reply 수정→Claude APPROVE→ledger APPROVED→execution 완료→Stop 가드 통과. + 음성 테스트(승인 前 쓰기 차단·미완료 종료 차단·정직한 미완료 보고 통과·active slug만).
...
- **공개 완료**: `github.com:qnamy/harnie` `main` = `9fcf719`(단일 커밋, 66 files, 테스트 115/115 pass). 로컬 브랜치 `feat/harnie-execution-state`도 같은 커밋(원하면 `main` 체크아웃 후 정리). C 구현 = Codex 크로스-모델 코드리뷰 12라운드 APPROVE(REJECT 11회 반영).
- **로컬 WIP(미커밋)**: `skills/comment-resolve/{SKILL.md,SKILL-ko.md}`·`skills/deploy-approval/{SKILL.md,SKILL-ko.md}` 4파일 — 영/한 번역 편집 중. main 이미 푸시됐으니 **amend 아닌 별도 커밋**으로 마무리.
- **바로 다음 순서**:
  1. WIP skills 2종 번역 마무리 → 커밋·push.
  2. **B (P0 게이트): 스왑 full-loop E2E** — throwaway git repo에서 실제 codex MCP(`mcp__codex__codex`/`-reply`)로 **Codex 빌더 REJECT→codex-reply 수정→Claude(`harnie-reviewer`) APPROVE→ledger APPROVED→`execution.mjs verify` receipt→`completion` complete→Stop 통과** 1사이클. 음성 확인: 승인 前 소스 쓰기 차단·미완료 종료 차단·정직한 미완료(`HARNIE_STATUS: INCOMPLETE`) 보고 통과.
...
- **검증 명령**: `node --test scripts/*.test.mjs hooks/*.test.mjs`(현재 115 pass).
