...
- **결과 — 기록만 하고 고치지 않은 것**: ① Codex 무인 모드(`-a never`)에서는 `captureTree`가 임시 git 인덱스를 `os.tmpdir()`에 만들지 못해 `init`·`runs`가 실패한다 — harnie run을 Codex에서 무인으로 돌릴 수 없다(지침은 `orca-dispatch.md`에서 정정, 코드는 0.14 범위 밖). ② 승인 전 M run은 manifest가 없어 `completion`이 `complete:true, noManifest:true`를 내고 Stop 훅도 통과시킨다 — 초기 커밋부터의 동작이며 0.14의 회귀가 아니다. ③ 두 M run 모두 승인 질문 직전에 `Invalid tool parameters`가 한 번씩 떴다(재현 조건 미확인, 결과적으로 재시도 후 정상 바인딩).
- **결과 — 부수 관측**: Stop 훅이 백그라운드 리뷰 대기 중의 조기 종료를 막고 블로커를 정확히 열거했다. §13-4의 읽기 마찰이 dev-solo에서 구조적 우회를 만든다 — 리뷰어를 부르는 명령줄에 상태 경로를 적을 수 없어 설계·코드 delta·원장이 전부 run 밖(`/private/tmp/…-handoff/`)으로 복사돼 전달된다. 리뷰가 run 안의 산출물이 아니라 그 사본을 보게 되므로, "감내한다"던 판단의 근거가 달라졌다(§13-4 재검토 대상).
...
| `.harnie` 읽기 마찰 | 완화하지 않고 감내한다. U7 실런에서 견디기 어려우면 별건 | §8 |
